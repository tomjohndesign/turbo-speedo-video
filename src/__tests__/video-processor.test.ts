...
describe("VideoProcessor", () => {
  ...
  describe("checkFfmpegAvailable", () => {
    it("should pass when FFmpeg is available", async () => {
      mockedExecFile.mockResolvedValue({ stdout: "ffmpeg version 4.4.0", stderr: "" } as any);
    ...
    it("should throw error when FFmpeg is not available", async () => {
      ...
      await expect(processor["checkFfmpegAvailable"]()).rejects.toThrow("FFmpeg is not installed or not available in PATH");
    });
  });
  ...
  describe("getVideoInfo", () => {
    ...
    it("should throw error for invalid file", async () => {
      ...
      await expect(processor["getVideoInfo"]("/invalid/path.mp4")).rejects.toThrow("Failed to analyze video file");
    });
  });

  describe("buildFfmpegCommand", () => {
    it("should build command for speed <= 2x with audio", () => {
      const command = processor["buildFfmpegCommand"]("/input.mp4", "/output.mp4", "2");
    ...
    it("should build command for speed > 2x with chained atempo filters", () => {
      const command = processor["buildFfmpegCommand"]("/input.mp4", "/output.mp4", "8");
    ...
    it("should build command for 10x speed with proper atempo chain", () => {
      const command = processor["buildFfmpegCommand"]("/input.mp4", "/output.mp4", "10");
  ...
  describe("buildFfmpegCommandNoAudio", () => {
    it("should build command without audio", () => {
      const command = processor["buildFfmpegCommandNoAudio"]("/input.mp4", "/output.mp4", "4");
  ...
  describe("processVideo", () => {
    it("should process video successfully", async () => {
      ...
      mockedExecFile
        .mockResolvedValueOnce({ stdout: "ffmpeg version 4.4.0", stderr: "" } as any) // ffmpeg -version
    ...
    it("should throw error for non-existent input file", async () => {
      ...
      await expect(processor.processVideo("/nonexistent.mp4", "2")).rejects.toThrow("Input file does not exist");
    });
    ...
    it("should throw error when output file already exists", async () => {
      ...
      await expect(processor.processVideo("/input.mp4", "2")).rejects.toThrow("Output file already exists");
    });
    ...
    it("should handle FFmpeg processing failure", async () => {
      ...
      await expect(processor.processVideo("/input.mp4", "2")).rejects.toThrow("FFmpeg processing failed");
    });
  });
